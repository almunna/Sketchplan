...
router.post(
  ...
  async (req, res) => {
    ...
    console.log("📥 Received Names from Body:", { ownerName, transferredTo });
      ...
      await Submission.updateOne(
        { submission_id },
        {
          $set: {
            ...
            ...(submission.status === "draft" && { status: "New" }),
          },
        }
      );
    }
    ...
    const drawStaticPage = async () => {
      ...
      const coords = {
        ...
        transferred: { x: 160, y: pageHeight - 125 },
      ...
      page.drawText(ownerName || submission.ownerName || "", {
        x: coords.registered.x + regW + 10,
      ...
      const transLabel = "Transferred to:";
      const transW = helvetica.widthOfTextAtSize(transLabel, fontSize);
      page.drawText(transferredTo || submission.transferredTo || "", {
        x: coords.transferred.x + transW + 10,
    ...
    if (satellitePage) {
      try {
        ...
        const imgX = marginX;
        const imgY = 200;
        const imgW = pageWidth - marginX * 3;
    ...
    const clientName = (ownerName || submission.ownerName || "Client").replace(
      /\s+/g,
      "_"
    );
    let fileName = "";
